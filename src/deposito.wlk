...
class Deposito {
	...
	method permitirMoverse(formacion) {
		if (!formacion.puedeMoverse()) {
			if (locomotorasSueltas.any{ locomotora => locomotora.arrastreUtil() >= formacion.arrastreFaltante() }) {
				formacion.agregarLocomotora(locomotorasSueltas.find{ locomotora => locomotora.arrastreUtil() >= formacion.arrastreFaltante()})
			}
		}
	}
